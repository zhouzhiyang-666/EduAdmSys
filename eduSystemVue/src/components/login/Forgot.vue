...
<div id="forgot" class="forgot_div">
  ...
  <el-form-item prop="user" label="用户名">
      <el-input v-model="userForm.user" @blur="delSpace"></el-input>
  </el-form-item>

  <el-form-item prop="tel" label="手机号">
      <el-input v-model="userForm.tel" maxlength="11" show-word-limit></el-input>
  </el-form-item>
...
import md5 from "js-md5"
  export default {
    data() {
      ...
      var validateTel = (rule, value, callback) => {
        if (value === '') {
          ...
          if(!this.$common.checkStr(value, 'mobile')){
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$axios
            .post('/public/ResetPwdByInfo/',this.userForm)
            .then((result)=> {
      ...
      getCode(){
        this.$axios
        .post('/public/getResetPwdCode/', { //获取验证码接口
            userid:this.userForm.user,
            phoneNumber:this.userForm.tel
        })
        .then((result)=> {
          if (result.data.code === 1) {
            this.$message.success('验证码已发送，10分钟内有效！')
						this.$set(this.userForm,'vcode',result.data.datas.phoneCode)
          }else{
            this.$message({
              type: 'info',
              message: result.data.msg
      ...
      delSpace(){
        setTimeout(()=>{
          this.userForm.user = this.userForm.user.replace(/\s*/g, '')
        },500)
      }
    }
  }
