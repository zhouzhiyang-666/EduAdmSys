...
<div style="width:1101px">
  ...
  <el-col :span="7">
    <label>教室：</label>
    <el-select v-model="roomSelected" placeholder="请选择教室" @change="getRoomSelected" style="width:180px; color: #000;">
    <el-option
  ...
  <el-col :span="2">
     <el-button type="primary" plain @click="selectOk">查询</el-button>
  </el-col>
  </el-row>
  <el-table
    :data="roomTableData"
    ...
    methods:{
      ...
      getAreaSelected(){
        ...
        .then((result)=> {
          ...
          this.roomSelected = ""
					this.roomOptions = []
          let length = result.data.datas.length
					for(let i=0;i<length;i++){
						this.roomOptions.push(result.data.datas[i])
					}
        })
        .catch((error)=> {
            alert(error)
      ...
      selectOk(){
        ...
        this.$axios
        .post('/public/getRoomCourse/', { //获取查询教室课表接口
